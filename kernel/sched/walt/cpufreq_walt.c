		...
		struct waltgov_cpu *wg_cpu, u64 time)
{
	...
	if (thermal_isolated_now) {
		if (!wg_policy->thermal_isolated) {
			...
			final_freq = 0;  /*staticoeed to change freq, i.e. continue with min freq */
		}
		raw_freq = final_freq;
		freq = raw_freq;
		goto out;
 ...
 * of a CPU. If allew IO wait boost is requested after more then a tick, then
 ...
 * each time an increase hasot been requested (!iowait_boost_pending).
 *
...
static unsigned int waltgov_next_freq_shared(struct waltgov_cpu *wg_cpu, u64 time)
{
	...
	if (eas_opt_enable && (util_thresh_percent[cluster_id] != 100) && count_cpu) {
		...
		avg_nr_running = avg_nr_running / count_cpu;
		util_orig = util;
