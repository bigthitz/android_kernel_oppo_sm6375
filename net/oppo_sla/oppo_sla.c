...
static int oppo_sla_vpn_connected = 0;
...
static int oppo_sla_def_net = 0;    //WLAN0->0 WLAN1->1 CELL->2
...
static struct oppo_dual_sta_info dual_wifi_app_list;
static struct oppo_dual_sta_info download_app_list;
static struct oppo_dual_sta_info vedio_app_list;
...
static void calc_network_congestion(void)
{
	...
	if (oppo_sla_info[MAIN_WLAN].wlan_score <= rom_update_info.wlan_bad_score) {
		rtt_num = 1;
	}
	...
	for (i = 0; i < IFACE_NUM; i++) {
		if (oppo_sla_info[i].if_up) {
			...
			if (oppo_sla_info[i].sla_rtt_num >= rtt_num) {
				...
				if (oppo_sla_info[i].download_flag < DOWNLOAD_FLAG) {
				...
				if (avg_rtt >= BACK_OFF_RTT_2) {
					...
					oppo_sla_info[i].download_speed /= 2;
...
static void init_dual_wifi_weight(void)
{
	...
	if (INIT_ACTIVE_TYPE != dual_wifi_active_type &&
		(oppo_sla_info[MAIN_WLAN].sla_avg_rtt >= BACK_OFF_RTT_2 ||
			oppo_sla_info[MAIN_WLAN].max_speed <= CALC_WEIGHT_MIN_SPEED_2)) {
		oppo_sla_info[MAIN_WLAN].weight = 0;
		oppo_sla_info[SECOND_WLAN].weight = 100;

	} else if (LOW_SPEED_HIGH_RTT == dual_wifi_active_type ||
		LOW_WLAN_SCORE == dual_wifi_active_type ||
		LOW_DL_SPEED == dual_wifi_active_type) {
		oppo_sla_info[MAIN_WLAN].weight = 30;
		oppo_sla_info[SECOND_WLAN].weight = 100;

	} else if (WLAN_DOWNLOAD == dual_wifi_active_type) {
		// for download active
		oppo_sla_info[MAIN_WLAN].weight = 50;
		oppo_sla_info[SECOND_WLAN].weight = 100;

	} else {
		// for manual active or networkReuqest
		init_weight_delay_count = 0;
		oppo_sla_info[MAIN_WLAN].weight = 100;
		oppo_sla_info[SECOND_WLAN].weight = 0;
	}

	printk("oppo_sla_weight:init_dual_wifi_weight [%d] [%d]",
		oppo_sla_info[MAIN_WLAN].weight, oppo_sla_info[SECOND_WLAN].weight);
}
...
static void init_wifi_cell_weight(void)
{
	...
	oppo_sla_info[MAIN_WLAN].weight = 0;
	oppo_sla_info[CELL_INDEX].weight = 100;
	printk("oppo_sla_weight:init_wifi_cell_weight [%d] [%d]",
		oppo_sla_info[MAIN_WLAN].weight, oppo_sla_info[CELL_INDEX].weight);
}

static void init_dual_wifi_cell_weight(void)
{
	if (oppo_sla_info[MAIN_WLAN].sla_avg_rtt >= BACK_OFF_RTT_2) {
		oppo_sla_info[MAIN_WLAN].weight = 0;

		if (oppo_sla_info[SECOND_WLAN].sla_avg_rtt >= BACK_OFF_RTT_2) {
			oppo_sla_info[SECOND_WLAN].weight = 0;

		} else {
			oppo_sla_info[SECOND_WLAN].weight = 30;
		}
		...
		if (oppo_sla_info[SECOND_WLAN].sla_avg_rtt >= BACK_OFF_RTT_2) {
			...
			oppo_sla_info[MAIN_WLAN].weight = 15;
			oppo_sla_info[SECOND_WLAN].weight = 30;
		}
...
static void init_oppo_sla_weight(struct timeval tv, int work_mode)
{
	last_weight_tv = tv;
...
static void send_enable_to_framework(int enable_mode, int active_type)
{
	...
	if (oppo_sla_info[MAIN_WLAN].need_up) {
		oppo_sla_info[MAIN_WLAN].if_up = 1;
	}

	if (oppo_sla_info[SECOND_WLAN].need_up) {
		oppo_sla_info[SECOND_WLAN].if_up = 1;
	}

	payload[0] = enable_mode;
	payload[1] = active_type;
	oppo_sla_send_to_user(SLA_ENABLE, (char *)payload, sizeof(payload));
	printk("oppo_sla_netlink:mode[%d] [%d] send SLA_ENABLE to user\n", enable_mode,
		active_type);
}

static int enable_oppo_sla_module(struct nlmsghdr *nlh)
{
	...
	init_oppo_sla_weight(tv, enable_type);
	...
	printk("oppo_sla_netlink: enable type = %d\n", enable_type);
	oppo_sla_send_to_user(SLA_ENABLED, (char *)&enable_type, sizeof(int));

	sla_write_unlock();
	return 0;
}

static int enable_oppo_limit_speed()
{
	sla_write_lock();
	rate_limit_info.rate_limit_enable = 1;
	printk("enable_oppo_limit_speed ");
	sla_write_unlock();
	return 0;
}

static int disable_oppo_limit_speed()
{
	sla_write_lock();
	rate_limit_info.rate_limit_enable = 0;
	printk("disable_oppo_limit_speed ");
	sla_write_unlock();
	return 0;
}

static int oppo_limit_uid_changed(struct nlmsghdr *nlh)
{
	int uid = -1;
	int *data = (int *)NLMSG_DATA(nlh);
	uid = data[0];
	printk("oppo_limit_uid_changed uid:%d\n", uid);
	sla_write_lock();
	...
	struct sk_buff *skb)
{
	...
	if (SLA_MODE_DUAL_WIFI == sla_work_mode &&
		...
		// for manual active
		if (INIT_ACTIVE_TYPE == dual_wifi_active_type) {
			if ((oppo_sla_info[MAIN_WLAN].max_speed < CALC_WEIGHT_MIN_SPEED_3 &&
					oppo_sla_info[SECOND_WLAN].max_speed < CALC_WEIGHT_MIN_SPEED_3 &&
					oppo_sla_info[MAIN_WLAN].sla_avg_rtt < NORMAL_RTT) ||
				(oppo_sla_info[MAIN_WLAN].download_flag < DOWNLOAD_FLAG &&
					oppo_sla_info[MAIN_WLAN].download_speed >= DUAL_WLAN_MAX_DOWNLOAD_SPEED)) {

				choose_mark = MAIN_WLAN_MARK;

			} else if (100 == oppo_sla_info[MAIN_WLAN].weight &&
				oppo_sla_info[MAIN_WLAN].download_flag >= DOWNLOAD_FLAG &&
				oppo_sla_info[SECOND_WLAN].download_flag < DOWNLOAD_FLAG) {
			...
			if ((init_weight_delay_count > 0 &&
				...
				(oppo_sla_info[MAIN_WLAN].download_flag < DOWNLOAD_FLAG &&
					oppo_sla_info[SECOND_WLAN].download_flag < DOWNLOAD_FLAG &&
					oppo_sla_info[MAIN_WLAN].download_speed >= MAX_WLAN_SPEED &&
					oppo_sla_info[SECOND_WLAN].download_speed >= MAX_WLAN_SPEED)) {
	...
	struct sk_buff *skb)
{
	...
	if (SLA_MODE_DUAL_WIFI == sla_work_mode &&
		...
		if (oppo_sla_info[MAIN_WLAN].download_speed >= MAX_WLAN_SPEED) {

			choose_mark = MAIN_WLAN_MARK;

		} else if (oppo_sla_info[SECOND_WLAN].download_speed > MAX_WLAN_SPEED ||
			(oppo_sla_info[MAIN_WLAN].download_speed <= VIDEO_SPEED &&
				((init_weight_delay_count > 0
						&& oppo_sla_info[MAIN_WLAN].dl_little_speed < DOWNLOAD_SPEED) ||
					(oppo_sla_info[SECOND_WLAN].download_speed > VIDEO_SPEED
						&& oppo_sla_info[SECOND_WLAN].sla_avg_rtt <= 150)))) {
...
static u32 get_skb_mark_by_weight(void)
{
	...
	for (i = 0; i < IFACE_NUM; i++) {
		if (oppo_sla_info[i].if_up &&
			oppo_sla_info[i].weight) {
			if (sla_random < (0x147AE15 * oppo_sla_info[i].weight)) {
				return oppo_sla_info[i].mark;
			}
		}
	}

	return oppo_sla_info[MAIN_WLAN].mark;
}
...
static void reset_oppo_sla_calc_speed(struct timeval tv)
{
	...
	if (time_interval >= 60 &&
		oppo_speed_info[MAIN_WLAN].speed_done) {

		oppo_sla_calc_speed = 0;
		oppo_speed_info[MAIN_WLAN].speed_done = 0;
		...
		if (NULL != th && th->syn &&
			...
			*/
			...
			if (tmp_mark != MAIN_WLAN_MARK) {
				...
				if (dev && !memcmp(dev->name, oppo_sla_info[MAIN_WLAN].dev_name,
						strlen(dev->name))) {
					if (oppo_sla_debug) {
					...
					if (sk) {
						sk->oppo_sla_mark = MAIN_WLAN_MARK;
						//sk->sk_mark = sk->oppo_sla_mark;
					}
...
static void game_app_switch_network(u32 game_type)
{
	...
	if (oppo_sla_info[MAIN_WLAN].wlan_score_bad_count >= WLAN_SCORE_BAD_NUM) {
		wlan_bad = true;
	}
	...
	if (cell_quality_good &&
		...
		if (!game_uid[index].switch_time ||
			...
			oppo_sla_send_to_user(SLA_SWITCH_GAME_NETWORK, (char *)game_bp_info,
				sizeof(game_bp_info));
			printk("oppo_sla_game_rtt:uid = %u,game rtt = %u,wlan_bad = %d,game_rx_bad = %d,changing to cellular...\n",
	...
	if (!wlan_bad &&
		...
		if (!game_uid[index].switch_time ||
			...
			oppo_sla_send_to_user(SLA_SWITCH_GAME_NETWORK, (char *)game_bp_info,
				sizeof(game_bp_info));
			printk("oppo_sla_game_rtt:uid = %u,game rtt = %u,wlan_bad = %d,game_rx_bad = %d,changing to wlan...\n",
				uid, game_rtt, wlan_bad, game_rx_bad);
			return;
		}
	}
	...
	if (isTx) {
		...
		if (udph->len >= (tx_offset + tx_len) &&
			memcmp(payload + tx_offset, game_params[gameType].tx_fixed_value,
				tx_len) == 0) {
			if (oppo_sla_debug) {
				printk("oppo_sla_game_rtt_detect:srcport[%d] this is game RTT Tx skb.\n",
					ntohs(udph->source));
			}
	...
	if (ct->oppo_app_type > 0 && ct->oppo_app_type < GAME_NUM) {
		...
		if (iph &&
			...
			if (GAME_CJZC == game_index &&
				...
				if (MAIN_WLAN_MARK == ct_mark && oppo_sla_info[MAIN_WLAN].wlan_score > 40) {
					return SLA_SKB_ACCEPT;
			...
			if (ct_mark && skb->mark &&
				...
				printk("oppo_sla_game_rtt:reset game ct proto= %u,srcport = %d,"
	...
	enum ip_conntrack_info ctinfo)
{
	...
	if (INIT_APP_TYPE == ct->oppo_app_type) {
		...
		if (oppo_sla_enable &&
			(oppo_sla_def_net == MAIN_WLAN || oppo_sla_def_net == SECOND_WLAN) &&
			CELL_MARK == game_uid[i].mark) {
			game_uid[i].cell_bytes += skb->len;
		}
...
static void record_sla_app_cell_bytes(struct nf_conn *ct, struct sk_buff *skb)
{
	...
	if (oppo_sla_info[CELL_INDEX].if_up) {

		if (!oppo_sla_info[MAIN_WLAN].if_up || oppo_sla_def_net == CELL_INDEX) {
	...
	struct sk_buff *skb)
{
	...
	if ((oppo_sla_info[MAIN_WLAN].if_up || oppo_sla_info[MAIN_WLAN].need_up) &&
		...
		if (NULL == ct || nf_ct_is_untracked(ct) || (0 == ct->mark)) {
			...
			if (dev && !memcmp(dev->name, oppo_sla_info[MAIN_WLAN].dev_name,
				...
				&& !memcmp(dev->name, oppo_sla_info[SECOND_WLAN].dev_name, strlen(dev->name))) {
				if (NF_INET_LOCAL_OUT == hook) {
					wlan1_tcp_tx++;
...
static unsigned int oppo_sla_game_rtt_calc(struct sk_buff *skb)
{
	int shift = 3;
	s64 time_now;
	u32 game_rtt = 0;
...
static bool is_sla_white_or_game_app(struct nf_conn *ct, struct sk_buff *skb)
{
	...
	if (ct->oppo_app_type > 0 &&
		ct->oppo_app_type < DUAL_STA_APP_BASE) { //game app skb
		return true;
	}
...
static bool is_need_change_dns_network(int index)
{
	...
	if (oppo_sla_info[index].weight &&
		WEIGHT_STATE_USELESS != oppo_sla_info[index].weight_state &&
		WEIGHT_STATE_SCORE_INVALID != oppo_sla_info[index].weight_state) {

		if (0 == oppo_sla_info[MAIN_WLAN].weight ||
			WEIGHT_STATE_USELESS == oppo_sla_info[MAIN_WLAN].weight_state ||
			WEIGHT_STATE_SCORE_INVALID == oppo_sla_info[MAIN_WLAN].weight_state ||
			(oppo_sla_info[index].max_speed >= 100 &&
				oppo_sla_info[MAIN_WLAN].wlan_score_bad_count >= WLAN_SCORE_BAD_NUM)) {
	...
	if (NULL != iph &&
		...
		if (SLA_MODE_DUAL_WIFI == sla_work_mode) {
			if (is_need_change_dns_network(SECOND_WLAN)) {
				...
				statistic_dns_send_info(SECOND_WLAN);
			}
...
static void detect_white_list_app_skb(struct sk_buff *skb)
{
	...
	*/
	if (SLA_MODE_DUAL_WIFI != sla_work_mode &&
		ct->oppo_app_type >= DUAL_STA_APP_BASE) {
		ct->oppo_app_type = INIT_APP_TYPE;
	}
	...
	if (INIT_APP_TYPE == ct->oppo_app_type) {
		...
		*/
		if (oppo_sla_info[CELL_INDEX].if_up) {
			if (!oppo_sla_info[MAIN_WLAN].if_up ||
				oppo_sla_def_net == CELL_INDEX) {
				index = ct->oppo_app_type - WHITE_APP_BASE;
	...
	kuid_t *app_uid)
{
	...
	if (ct->oppo_app_type >= DUAL_STA_APP_BASE) {
		return true;
	}

	if (UNKNOW_APP_TYPE != ct->oppo_app_type) {

		last_type = ct->oppo_app_type;
		...
		for (i = 0; i < dual_wifi_app_list.count; i++) {
			if (dual_wifi_app_list.uid[i]) {
				if((uid.val % UID_MASK) == (dual_wifi_app_list.uid[i] % UID_MASK)) {
					ct->oppo_app_type = i + DUAL_STA_APP_BASE;
					return true;
				}
			}
		}

		ct->oppo_app_type = last_type;
	}
		...
		if (ctinfo == IP_CT_NEW) {
			...
			if (sk && sk_fullsock(sk)) {
				...
				if (NULL != iph &&
					...
					printk("oppo_sla_stream: screen_on[%d] uid[%u]"
...
static int sla_mark_skb(struct sk_buff *skb, const struct nf_hook_state *state)
{
	...
	if (oppo_sla_def_net == CELL_INDEX) {
		return NF_ACCEPT;
	}
...
static unsigned int oppo_sla_speed_calc(struct sk_buff *skb)
{
	enum ip_conntrack_info ctinfo;
	struct nf_conn *ct = NULL;
	int index = -1;
	...
	if (!sla_switch_enable || oppo_sla_enable) {
		return NF_ACCEPT;
	}

	//only calc wlan speed
	if (oppo_sla_info[MAIN_WLAN].if_up &&
		!oppo_sla_info[CELL_INDEX].if_up) {
		index = MAIN_WLAN;
					...
					if (oppo_speed_info[index].ms_speed_flag) {
						...
						if (tmp_time > 500000000) {
							...
							if (cell_quality_good &&
								...
								send_enable_to_framework(SLA_MODE_WIFI_CELL, INIT_ACTIVE_TYPE);
								printk("oppo_sla_netlink: calc speed is small,send enable sla to user\n");
							}
...
*/
static unsigned int oppo_sla_output_hook(void *priv,
	struct sk_buff *skb,
	const struct nf_hook_state *state)
{
	int ret = NF_ACCEPT;
...
static unsigned int oppo_sla_input_hook(void *priv,
	struct sk_buff *skb,
	const struct nf_hook_state *state)
{
	//add for android Q statictis tcp tx and tx
	...
	oppo_sla_speed_calc(skb);
	oppo_sla_game_rtt_calc(skb);
...
static void oppo_statistic_dev_rtt(struct sock *sk, long rtt)
{
	...
	if (oppo_sla_def_net == CELL_INDEX) {
		...
		} else if (oppo_sla_info[CELL_INDEX].if_up) {
			index = CELL_INDEX;
		}
...
static void is_need_calc_wlan_small_speed(int speed)
{
	if (sla_screen_on &&
		...
		if (speed <= 100 &&
			...
			oppo_sla_info[MAIN_WLAN].minute_speed > MINUTE_LITTE_RATE) {

			printk("oppo_sla_speed: detect speed is little\n");
			oppo_sla_calc_speed = 1;
			memset(&oppo_speed_info[MAIN_WLAN], 0x0, sizeof(struct oppo_speed_calc));
		}
	}
}
...
static bool is_wlan_speed_good(int index)
{
	...
	if (oppo_sla_info[i].max_speed >= 300 &&
		oppo_sla_info[i].sla_avg_rtt < 150 &&
		oppo_sla_info[i].wlan_score >= 60 &&
		oppo_sla_info[i].download_flag < DOWNLOAD_FLAG) {
		ret = true;
	}
...
static void auto_disable_sla_for_good_wlan(void)
{
	...
	if (oppo_sla_enable &&
		...
		oppo_sla_info[MAIN_WLAN].sla_avg_rtt < 150 &&
		oppo_sla_info[MAIN_WLAN].max_speed >= 300) {

		enable_cell_to_user = false;
		oppo_sla_send_to_user(SLA_DISABLE, (char *)&work_mode, sizeof(int));

		sla_write_lock();
		oppo_sla_info[MAIN_WLAN].weight = 100;
		oppo_sla_info[CELL_INDEX].weight = 0;
		sla_write_unlock();
...
static void auto_disable_dual_wifi(void)
{
	...
	if (oppo_sla_enable &&
		...
		oppo_sla_info[SECOND_WLAN].minute_speed < MINUTE_LITTE_RATE) {
		...
		printk("oppo_sla_netlink:speed good,disable dual wifi\n");
	}
}
...
static void detect_wlan_state_with_speed(void)
{
	...
	is_need_calc_wlan_small_speed(oppo_sla_info[MAIN_WLAN].max_speed);
}
...
static void reset_network_state_by_speed(int index, int speed)
{
	...
	if (speed >= 50 &&
		WEIGHT_STATE_USELESS == oppo_sla_info[i].weight_state) {
...
static void detect_dl_little_speed(int index, int speed)
{
	...
	if (speed >= CALC_WEIGHT_MIN_SPEED_2) {
		if (oppo_sla_info[i].little_speed_num < ADJUST_SPEED_NUM) {
			oppo_sla_info[i].little_speed_num++;

			if (speed > oppo_sla_info[i].tmp_little_speed) {
				oppo_sla_info[i].tmp_little_speed = speed;
			}

			if (ADJUST_SPEED_NUM == oppo_sla_info[i].little_speed_num) {
				oppo_sla_info[i].dl_little_speed = oppo_sla_info[i].tmp_little_speed;
			}
		}

	} else {
		oppo_sla_info[i].tmp_little_speed = 0;
		oppo_sla_info[i].little_speed_num = 0;
	}
}
...
static void detect_network_download(int index, int speed)
{
	...
	if (speed > DOWNLOAD_SPEED) {
		if (oppo_sla_info[i].download_num < ADJUST_SPEED_NUM) {
			oppo_sla_info[i].download_num++;

			if (speed > oppo_sla_info[i].dl_mx_speed) {
				oppo_sla_info[i].dl_mx_speed = speed;
			}
		}

		//Adjust the speed to prevent speed burst leading to high speed
		if (ADJUST_SPEED_NUM == oppo_sla_info[i].download_num) {

			if (oppo_sla_info[i].download_speed > oppo_sla_info[i].dl_mx_speed) {
				oppo_sla_info[i].download_speed += oppo_sla_info[i].dl_mx_speed;
				oppo_sla_info[i].download_speed /= 2;
			...
			if (oppo_sla_info[i].max_speed > oppo_sla_info[i].download_speed) {
				oppo_sla_info[i].max_speed = oppo_sla_info[i].download_speed;
			}

			oppo_sla_info[i].download_num = 0;
			oppo_sla_info[i].dl_mx_speed = 0;

			if (oppo_sla_debug) {
				printk("oppo_sla_speed: adjust download speed = %d\n",
					oppo_sla_info[i].download_speed);
			}
		}

		if (speed > (oppo_sla_info[i].download_speed / 2)) {
			if (speed < rom_update_info.dual_wlan_download_speed &&
				oppo_sla_info[i].download_speed < rom_update_info.dual_wlan_download_speed &&
				oppo_sla_info[i].dual_wifi_download < (2 * DOWNLOAD_FLAG)) {

				oppo_sla_info[i].dual_wifi_download++;

			} else if (speed >= rom_update_info.dual_wlan_download_speed ||
				oppo_sla_info[i].download_speed >= rom_update_info.dual_wlan_download_speed) {

				if (oppo_sla_info[i].dual_wifi_download >= 2) {
					oppo_sla_info[i].dual_wifi_download -= 2;

				} else if (oppo_sla_info[i].dual_wifi_download) {
					oppo_sla_info[i].dual_wifi_download--;
				}
			}

			if (oppo_sla_info[i].download_flag < (2 * DOWNLOAD_FLAG)) {
	...
	if (speed < (oppo_sla_info[i].download_speed / 3)) {
...
static void adjust_speed_with_romupdate_param(int index)
{
	...
	if (MAIN_WLAN == i || SECOND_WLAN == i) {
		if (oppo_sla_info[i].wlan_score <= rom_update_info.wlan_bad_score &&
			...
			oppo_sla_info[i].max_speed > rom_update_info.second_wlan_speed) {
			oppo_sla_info[i].max_speed = rom_update_info.second_wlan_speed;
		}
	}
...
static void calc_download_speed(int index, u64 total_bytes, int time)
{
	...
	if (time >= DOWNLOAD_SPEED_TIME) {
		if (0 == oppo_sla_info[i].dl_total_bytes ||
			oppo_sla_info[i].dl_total_bytes > total_bytes) {

			oppo_sla_info[i].dl_total_bytes = total_bytes;

		} else {
			bytes = total_bytes - oppo_sla_info[i].dl_total_bytes;
			dl_speed = bytes / time;
			...
			if (dl_speed > oppo_sla_info[i].download_speed) {
				oppo_sla_info[i].left_speed = dl_speed - oppo_sla_info[i].download_speed;

			} else {
				oppo_sla_info[i].left_speed = oppo_sla_info[i].download_speed - dl_speed;
			}

			if (oppo_sla_debug) {
				printk("oppo_sla: cur download speed[%d] = %d\n", i, dl_speed);
			}
		}
	}
}
	...
	for (i = 0; i < IFACE_NUM; i++) {
		if (oppo_sla_info[i].if_up || oppo_sla_info[i].need_up) {
			...
			if (dev) {
				if (dev_isalive(dev) &&
					...
					if (0 == oppo_sla_info[i].total_bytes ||
						...
						if (temp_speed > oppo_sla_info[i].download_speed) {
							oppo_sla_info[i].download_speed = temp_speed;
						}
			...
			if (oppo_sla_debug /*&& net_ratelimit()*/) {
				printk("oppo_sla: dev_name = %s,if_up = %d,max_speed = %d,"
					...
					oppo_sla_info[i].sla_avg_rtt, oppo_sla_info[i].congestion_flag,
					oppo_sla_info[i].download_flag, oppo_sla_info[i].syn_retran,
					oppo_sla_info[i].minute_speed, oppo_sla_info[i].weight_state,
					oppo_sla_info[i].download_speed, oppo_sla_info[i].dual_wifi_download,
					oppo_sla_info[i].dl_little_speed);
			}
...
static void reset_invalid_network_info(struct oppo_dev_info *node)
{
	struct timeval tv;
	...
	if (oppo_sla_debug) {
		printk("oppo_sla: reset_invalid_network_info,dev_name = %s\n", node->dev_name);
	}
}
...
static int calc_weight_with_left_speed(int network_1, int network_2)
{
	...
	if ((oppo_sla_info[index1].download_flag >= DOWNLOAD_FLAG ||
			oppo_sla_info[index2].download_flag >= DOWNLOAD_FLAG) &&
		(oppo_sla_info[index1].max_speed > CALC_WEIGHT_MIN_SPEED_2 &&
			oppo_sla_info[index2].max_speed > CALC_WEIGHT_MIN_SPEED_2)) {

		if (oppo_sla_info[index1].download_flag >= DOWNLOAD_FLAG) {
			if (is_wlan_speed_good(index2) &&
				oppo_sla_info[index1].download_speed <= MAX_WLAN_SPEED) {
				if (oppo_sla_debug) {
					printk("oppo_sla_weight:network1 is download and network2 is much better\n");
				}

				oppo_sla_info[index1].weight = 0;
				oppo_sla_info[index2].weight = 100;
				return 1;
			}

			if (oppo_sla_info[index2].max_speed <= CALC_WEIGHT_MIN_SPEED_3) {
				if (oppo_sla_info[index2].avg_rtt >= rom_update_info.sla_rtt) {
					if (oppo_sla_debug) {
						printk("oppo_sla_weight:network1 is download but network1 is very bad\n");
					}

					oppo_sla_info[index1].weight = 100;
					oppo_sla_info[index2].weight = 0;
					return 1;
				}

				speed1 = oppo_sla_info[index1].max_speed;

			} else {
				speed1 = oppo_sla_info[index1].left_speed;
			}
		...
		if (oppo_sla_info[index2].download_flag >= DOWNLOAD_FLAG) {
			if (is_wlan_speed_good(index1) &&
				oppo_sla_info[index2].download_speed <= MAX_WLAN_SPEED) {
				if (oppo_sla_debug) {
					printk("oppo_sla_weight:network2 is download and network1 is much better\n");
				}
			...
			if (oppo_sla_info[index1].max_speed <= CALC_WEIGHT_MIN_SPEED_3) {
				if (oppo_sla_info[index1].avg_rtt >= rom_update_info.sla_rtt) {
					if (oppo_sla_debug) {
						printk("oppo_sla_weight:network2 is download but network1 is very bad\n");
					}
				...
				speed2 = oppo_sla_info[index2].max_speed;

			} else {
				speed2 = oppo_sla_info[index2].left_speed;
			}
		...
		oppo_sla_info[index1].weight = calc_weight_with_speed(speed1, speed2);
		oppo_sla_info[index2].weight = 100;

		if (oppo_sla_debug) {
			printk("oppo_sla_weight: calc weight with download speed\n");
		}
...
static int calc_weight_with_weight_state(int network_1, int network_2)
{
	...
	if ((WEIGHT_STATE_USELESS == oppo_sla_info[index1].weight_state ||
			WEIGHT_STATE_SCORE_INVALID == oppo_sla_info[index1].weight_state) &&
		(WEIGHT_STATE_USELESS != oppo_sla_info[index2].weight_state &&
			WEIGHT_STATE_SCORE_INVALID != oppo_sla_info[index2].weight_state)) {

		oppo_sla_info[index1].weight = 0;
		oppo_sla_info[index2].weight = 100;

		if (oppo_sla_debug) {
			printk("oppo_sla_weight: [%d] calc weight with WEIGHT_STATE_USELESS \n",
				index1);
		}

		return 1;

	} else if ((WEIGHT_STATE_USELESS != oppo_sla_info[index1].weight_state &&
			WEIGHT_STATE_SCORE_INVALID != oppo_sla_info[index1].weight_state) &&
		(WEIGHT_STATE_USELESS == oppo_sla_info[index2].weight_state ||
			WEIGHT_STATE_SCORE_INVALID == oppo_sla_info[index2].weight_state)) {
...
static int calc_weight_with_little_speed(int network_1, int network_2)
{
	...
	if ((is_wlan_speed_good(index2) &&
			oppo_sla_info[index1].max_speed <= CALC_WEIGHT_MIN_SPEED_2) ||
		(oppo_sla_info[index1].max_speed < CALC_WEIGHT_MIN_SPEED_1 &&
			CONGESTION_LEVEL_HIGH == oppo_sla_info[index1].congestion_flag)) {

		oppo_sla_info[index1].weight = 0;
		oppo_sla_info[index2].weight = 100;

		if (oppo_sla_debug) {
			printk("oppo_sla_weight:calc weight with little speed \n");
		}
...
static bool is_main_wlan_poor(void)
{
	...
	if (oppo_sla_info[index].wlan_score > 10 &&
		oppo_sla_info[index].wlan_score <= score) {
		return true;
	}

	if (oppo_sla_info[index].download_flag >= DOWNLOAD_FLAG) {
		return true;
	}

	if (oppo_sla_info[index].max_speed <= rom_update_info.sla_speed &&
		oppo_sla_info[index].sla_avg_rtt >= rom_update_info.dual_wifi_rtt) {
		return true;
	}

	if (oppo_sla_info[index].dl_little_speed < rom_update_info.sla_speed &&
		oppo_sla_info[index].download_speed < VIDEO_SPEED) {
		return true;
	}
...
static void recalc_two_network_weight(int network_1, int network_2)
{
	...
	int speed1 = oppo_sla_info[index1].max_speed;
	int speed2 = oppo_sla_info[index2].max_speed;

	if (oppo_sla_info[index1].if_up &&
		oppo_sla_info[index2].if_up) {
		...
		if ((speed1 >= CALC_WEIGHT_MIN_SPEED_2 &&
			...
			(CONGESTION_LEVEL_HIGH == oppo_sla_info[index1].congestion_flag ||
				CONGESTION_LEVEL_HIGH == oppo_sla_info[index2].congestion_flag) ||
			((CONGESTION_LEVEL_MIDDLE == oppo_sla_info[index1].congestion_flag ||
					CONGESTION_LEVEL_MIDDLE == oppo_sla_info[index2].congestion_flag) &&
			...
			if (SLA_MODE_DUAL_WIFI == sla_work_mode &&
				...
				oppo_sla_info[index1].weight = 30;
				oppo_sla_info[index2].weight = 100;

			} else if (SLA_MODE_WIFI_CELL == sla_work_mode) {
				oppo_sla_info[index1].weight = 15;
				oppo_sla_info[index2].weight = 100;
			}
	...
	} else if (oppo_sla_info[index1].if_up) {
		oppo_sla_info[index1].weight = 100;
	}
...
calc_weight_finish:
	...
	if (oppo_sla_debug) {
		printk("oppo_sla_weight: work_mode = %d,weight[%d] = %d,weight[%d] = %d\n",
			sla_work_mode, index1, oppo_sla_info[index1].weight, index2,
			oppo_sla_info[index2].weight);
	}
...
static void recalc_dual_wifi_cell_weight(void)
{
	int i = 0;
	int sum_speed = 0;
	int speed1 = oppo_sla_info[0].max_speed;
	int speed2 = oppo_sla_info[1].max_speed;
	int speed3 = oppo_sla_info[2].max_speed;
	...
	if ((speed1 >= CALC_WEIGHT_MIN_SPEED_2 &&
			speed2 >= CALC_WEIGHT_MIN_SPEED_2 &&
			speed2 >= CALC_WEIGHT_MIN_SPEED_2) ||
		(oppo_sla_info[0].congestion_flag > CONGESTION_LEVEL_NORMAL &&
			oppo_sla_info[1].congestion_flag > CONGESTION_LEVEL_NORMAL &&
			oppo_sla_info[2].congestion_flag > CONGESTION_LEVEL_NORMAL)) {

		sum_speed = speed1 + speed2 + speed3;
		oppo_sla_info[0].weight = (100 * speed1) / sum_speed;
		oppo_sla_info[1].weight = (100 * (speed1 + speed2)) / sum_speed;
		oppo_sla_info[2].weight = 100;
		goto finish_calc;
	}
...
finish_calc:
	...
	if (oppo_sla_debug) {
		printk("oppo_sla_weight: work_mode = %d,weight[0] = %d,weight[1] = %d,weight[2] = %d\n",
			sla_work_mode, oppo_sla_info[0].weight, oppo_sla_info[1].weight,
			oppo_sla_info[2].weight);
	}
	...
	for (i = 0; i < WLAN_NUM; i++) {
		if (oppo_sla_info[i].need_up) {
			...
			if ((tv.tv_sec - calc_wlan_rtt_tv.tv_sec) >= time) {
				...
				oppo_sla_info[i].if_up = 1;
				oppo_sla_info[i].need_up = false;

				printk("oppo_sla:wlan[%d]time[%d] up wlan iface actually\n", i, time);
			}
		}
	}
}
...
static void send_speed_and_rtt_to_user(void)
{
	...
	if (oppo_sla_info[MAIN_WLAN].if_up || oppo_sla_info[MAIN_WLAN].need_up ||
		...
		payload[0] = oppo_sla_info[MAIN_WLAN].max_speed;
		payload[1] = oppo_sla_info[MAIN_WLAN].avg_rtt;

		payload[2] = oppo_sla_info[SECOND_WLAN].max_speed;
		payload[3] = oppo_sla_info[SECOND_WLAN].avg_rtt;

		payload[4] = oppo_sla_info[CELL_INDEX].max_speed;
		payload[5] = oppo_sla_info[CELL_INDEX].avg_rtt;

		payload[6] = sla_work_mode;
		payload[7] = oppo_sla_info[MAIN_WLAN].download_speed;
		...
		ret = oppo_sla_send_to_user(SLA_NOTIFY_SPEED_RTT, (char *) total_payload,
				sizeof(total_payload));

		if (oppo_sla_debug) {
			printk("oppo_sla:send_speed_and_rtt_to_user wlan0:max_speed=%d, avg_rtt=%d  "
...
static int wlan_can_enable_second_wifi(int index)
{
	...
	if (oppo_sla_info[index].if_up ||
		oppo_sla_info[index].need_up) {
		int score = 10 + rom_update_info.dual_wlan_bad_score;
		...
		if (oppo_sla_info[index].dual_wifi_download >= DOWNLOAD_FLAG &&
			oppo_sla_info[index].download_speed <
		...
		if (oppo_sla_info[index].wlan_score > 10 &&
			oppo_sla_info[index].wlan_score <= score) {
			printk("oppo_sla: send enable dual wifi with score[%d]\n", score);
		...
		if (oppo_sla_info[index].dl_little_speed < rom_update_info.sla_speed &&
			oppo_sla_info[index].download_speed < VIDEO_SPEED) {
			printk("oppo_sla:dl_little_speed = %d\n", oppo_sla_info[index].dl_little_speed);
...
static bool wlan_can_enable_cell(int index)
{
	if (oppo_sla_info[index].if_up ||
		...
		if (oppo_sla_info[index].download_flag < DOWNLOAD_FLAG) {
			...
			if (SLA_MODE_DUAL_WIFI != sla_work_mode &&
				oppo_sla_info[index].wlan_score > 10 &&
				oppo_sla_info[index].wlan_score <= score) {
				printk("oppo_sla: send enable sla with score[%d]\n", score);
				return true;
			}

			if (oppo_sla_info[index].max_speed <= rom_update_info.sla_speed &&
				(oppo_sla_info[index].sla_avg_rtt >= rom_update_info.sla_rtt ||
					oppo_sla_info[index].wlan_score_bad_count >= WLAN_SCORE_BAD_NUM)) {
...
static void sla_show_dailog(void)
{
	...
	if (sla_screen_on &&
		...
		for (i = 0; i < WLAN_NUM; i++) {
			if (oppo_sla_info[i].if_up &&
				oppo_sla_info[i].download_flag < DOWNLOAD_FLAG &&
				oppo_sla_info[i].sla_avg_rtt >= rom_update_info.sla_rtt) {
		...
		if (wlan_bad) {
			...
			if (oppo_sla_debug) {
				printk("oppo_sla_netlink:show dailog now\n");
			}
		}
...
static bool dual_wlan_enable_cell(void)
{
	...
	if (wlan_can_enable_cell(MAIN_WLAN) &&
		...
		if (oppo_sla_enable) {
			printk("oppo_sla_netlink:[0] dual sta enable cell\n");
		}

		return true;

	} else if (!init_weight_delay_count) {
		sum_speed = oppo_sla_info[MAIN_WLAN].max_speed +
			oppo_sla_info[SECOND_WLAN].max_speed;

		if (sum_speed <= rom_update_info.sla_speed &&
			(oppo_sla_info[MAIN_WLAN].sla_avg_rtt >= rom_update_info.sla_rtt ||
				oppo_sla_info[SECOND_WLAN].sla_avg_rtt >= rom_update_info.sla_rtt)) {

			if (oppo_sla_enable) {
				printk("oppo_sla_netlink:[1] dual sta enable cell\n");
			}
...
static void sla_detect_should_enable(void)
{
	...
	if (sla_screen_on && CELL_INDEX != oppo_sla_def_net) {
		...
		if (oppo_sla_info[index].if_up || oppo_sla_info[index].need_up) {
			...
			if (oppo_sla_debug) {
				printk("oppo_sla_rtt: index = %d,wlan_rtt = %d,"
					...
					"enable_cell_to_user = %d,enable_wifi_to_user = %d,sla_switch_enable = %d,oppo_sla_enable= %d,"
					"oppo_sla_def_net = %d,sla_avg_rtt = %d,detect_mode = %d,work_mode = %d,game_cell_to_wifi = %d\n",
					index, oppo_sla_info[index].avg_rtt,
					oppo_sla_info[index].wlan_score_bad_count,
					cell_quality_good, need_show_dailog, sla_screen_on,
					enable_cell_to_user, enable_second_wifi_to_user, sla_switch_enable,
					oppo_sla_enable, oppo_sla_def_net, avg_rtt, sla_detect_mode, sla_work_mode,
					game_cell_to_wifi);
			}
		}
	}
...
static void game_rx_update(void)
{
	if (!oppo_sla_vpn_connected &&
		...
		if (udp_rx_show_toast) {
			oppo_sla_send_to_user(SLA_NOTIFY_GAME_RX_PKT, (char *)&delta_udp_pkts,
...
static void dns_statistic_timer(struct timeval tv)
{
	...
	for (i = 0; i < IFACE_NUM; i++) {
		if (dns_info[i].in_timer &&
			...
			reset_invalid_network_info(&oppo_sla_info[i]);
			...
			printk("oppo_sla_dns: detec dns[%d] error\n", i);
		}
	}
...
static void oppo_sla_timer_function(struct timer_list *t)
{
	if (workqueue_sla) {
...
#else
	timer_setup(&sla_timer, oppo_sla_timer_function, 0);
#endif
...
static struct ctl_table oppo_sla_sysctl_table[] = {
	...
	{
		.procname	= "oppo_sla_vpn_connected",
		.data		= &oppo_sla_vpn_connected,
		.maxlen		= sizeof(int),
		.mode		= 0644,
		.proc_handler	= proc_dointvec,
	},
...
static int disable_oppo_sla_module(struct nlmsghdr *nlh)
{
	int disable_type = 0;

	int *data = (int *)NLMSG_DATA(nlh);
	disable_type = data[0];
	printk("oppo_sla_netlink: type[%d] disable,oppo_sla_enable[%d],work_mode[%d]\n",
		disable_type, oppo_sla_enable, sla_work_mode);

	sla_write_lock();

	if (oppo_sla_enable && disable_type) {
		...
		if (disable_type == sla_work_mode) {
			...
			init_game_online_info();
			printk("oppo_sla_netlink: type[%d] disabled\n", disable_type);
		}

		oppo_sla_send_to_user(SLA_DISABLED, (char *)&disable_type, sizeof(int));
	}
...
static int oppo_sla_iface_changed(struct nlmsghdr *nlh)
{
	int index = -1;
	int up = 0;
	char *p;
	...
	up = data[1];
	p = (char *)(data + 2);
	printk("oppo_sla_netlink:oppo_sla_iface_changed index:%d, up:%d, ifname:%s\n",
		index, up, p);
	...
	if (index >= 0 && index < IFACE_NUM) {
		...
		if (up) {
			sla_write_lock();
			oppo_sla_info[index].if_up = 0;

			if (index == MAIN_WLAN || index == SECOND_WLAN) {
				if (index == MAIN_WLAN) {
					mark = MAIN_WLAN_MARK;
					oppo_sla_info[MAIN_WLAN].need_up = true;
				...
				calc_wlan_rtt_tv = tv;
				last_download_speed_tv = tv;
				oppo_sla_info[CELL_INDEX].max_speed = 0;
				oppo_sla_info[index].dl_little_speed = DOWNLOAD_SPEED;

			} else if (index == CELL_INDEX) {
				mark = CELL_MARK;
				oppo_sla_info[CELL_INDEX].if_up = 1;
				oppo_sla_info[CELL_INDEX].wlan_score = WLAN_SCORE_GOOD;
			}
...
static int oppo_sla_set_primary_wifi(struct nlmsghdr *nlh)
{
	...
	if (*data == WLAN0_INDEX || *data == WLAN1_INDEX) {
		...
		printk("oppo_sla_netlink: oppo_sla_set_primary_wifi main_wlan[%d],second_wlan[%d]\n",
			MAIN_WLAN, SECOND_WLAN);

	} else {
		printk("oppo_sla_netlink: oppo_sla_set_primary_wifi invalid index = %d\n",
			*data);
	}
...
static void set_weight_state_by_score(int index, int score)
{
	if (index >= 0 &&
		...
		if (WLAN_NETWORK_INVALID == score &&
			WEIGHT_STATE_SCORE_INVALID != oppo_sla_info[index].weight_state) {
			printk("oppo_sla_score: network[%d] invalid\n", index);
			reset_invalid_network_info(&oppo_sla_info[index]);
			oppo_sla_info[index].weight_state = WEIGHT_STATE_SCORE_INVALID;
		...
		} else if (!oppo_sla_info[index].wlan_score_bad_count &&
			WEIGHT_STATE_SCORE_INVALID == oppo_sla_info[index].weight_state) {
			printk("oppo_sla_score: network[%d] reset to normal\n", index);
			oppo_sla_info[index].weight_state = WEIGHT_STATE_NORMAL;
		}
	}
...
static int oppo_sla_set_wifi_score(struct nlmsghdr *nlh)
{
	int *data = (int *)NLMSG_DATA(nlh);
	...
	if (NULL != data &&
		...
		if (score <= (rom_update_info.wlan_bad_score - 5)) {
			...
			oppo_sla_info[index].wlan_score_bad_count += 3;

		} else if (score >= rom_update_info.wlan_good_score) {
			oppo_sla_info[index].wlan_score_bad_count = 0;
		...
		if (oppo_sla_info[index].wlan_score_bad_count > (2 * WLAN_SCORE_BAD_NUM)) {
			oppo_sla_info[index].wlan_score_bad_count = 2 * WLAN_SCORE_BAD_NUM;
		}

	} else {
		printk("oppo_sla_netlink:oppo_sla_set_wifi_score invalid message!\n");
	}
...
static int oppo_sla_set_game_rtt_detecting(struct nlmsghdr *nlh)
{
	if (SLA_ENABLE_GAME_RTT == nlh->nlmsg_type) {
		oppo_sla_vpn_connected = 1;

	} else {
		oppo_sla_vpn_connected = 0;
	}
...
static int oppo_sla_set_switch_state(struct nlmsghdr *nlh)
{
	...
	printk("oppo_sla_netlink:sla switch sla_enable = %d,dual_wifi_enable = %d\n",
...
static int oppo_sla_set_params(struct nlmsghdr *nlh)
{
	...
	if (count == 13) {
		...
		printk("oppo_sla_netlink:set params count=%u params[0] = %u,params[1] = %u,"
...
static int oppo_sla_set_dual_wifi_app_uid(struct nlmsghdr *nlh)
{
	u32 *info = (u32 *)NLMSG_DATA(nlh);
	memset(&dual_wifi_app_list, 0x0, sizeof(struct oppo_dual_sta_info));
	...
	if (dual_wifi_app_list.count > 0
		...
		for (i = 0; i < dual_wifi_app_list.count; i++) {
			...
			printk("oppo_sla_netlink:set_dual_wifi_app_uid count=%d, uid[%d]=%d\n",
...
static int oppo_sla_set_download_app_uid(struct nlmsghdr *nlh)
{
	u32 *info = (u32 *)NLMSG_DATA(nlh);
	memset(&download_app_list, 0x0, sizeof(struct oppo_dual_sta_info));
	...
	if (download_app_list.count > 0 && download_app_list.count < DUAL_STA_APP_NUM) {
		...
		for (i = 0; i < download_app_list.count; i++) {
			...
			printk("oppo_sla_netlink:set_download_app_uid count=%d, uid[%d]=%d\n",
...
static int oppo_sla_set_vedio_app_uid(struct nlmsghdr *nlh)
{
	u32 *info = (u32 *)NLMSG_DATA(nlh);
	memset(&vedio_app_list, 0x0, sizeof(struct oppo_dual_sta_info));
	...
	if (vedio_app_list.count > 0 && vedio_app_list.count < DUAL_STA_APP_NUM) {
		...
		for (i = 0; i < vedio_app_list.count; i++) {
			...
			printk("oppo_sla_netlink:vedio_app_list count=%d, uid[%d]=%d\n",
...
static void oppo_sla_set_vpn_state(struct nlmsghdr *nlh)
{
	int *data = (int *)NLMSG_DATA(nlh);

	/*1 ---> vpn connected
	 *0 ---> vpn disconnected*/
	oppo_sla_vpn_connected = data[0];

	printk("oppo_sla_netlink:set vpn connecet state = %d\n", data[0]);
}
...
static void reset_main_wlan_info(void)
{
	...
	if (oppo_sla_info[MAIN_WLAN].if_up) {
		...
		oppo_sla_info[MAIN_WLAN].sum_rtt = 0;
		oppo_sla_info[MAIN_WLAN].avg_rtt = 0;
		oppo_sla_info[MAIN_WLAN].rtt_index = 0;
		oppo_sla_info[MAIN_WLAN].sla_avg_rtt = 0;
		oppo_sla_info[MAIN_WLAN].sla_rtt_num = 0;
		oppo_sla_info[MAIN_WLAN].sla_sum_rtt = 0;
		oppo_sla_info[MAIN_WLAN].syn_retran = 0;
		oppo_sla_info[MAIN_WLAN].max_speed /= 2;
		oppo_sla_info[MAIN_WLAN].left_speed = 0;
		oppo_sla_info[MAIN_WLAN].current_speed = 0;
		oppo_sla_info[MAIN_WLAN].minute_speed = MINUTE_LITTE_RATE;
		oppo_sla_info[MAIN_WLAN].congestion_flag = CONGESTION_LEVEL_NORMAL;
	}
}
...
static void oppo_sla_init_weight_by_wlan_assi(void)
{
	...
	printk("oppo_sla_weight:init_weight_by_wlan_assi[%d] [%d]\n",
		oppo_sla_info[MAIN_WLAN].weight, oppo_sla_info[CELL_INDEX].weight);
	sla_write_unlock();
}
	...
	struct netlink_ext_ack *extack)
{
	...
	case SLA_ENABLE:
		ret = enable_oppo_sla_module(nlh);
		break;

	case SLA_DISABLE:
		ret = disable_oppo_sla_module(nlh);
		break;

	case SLA_IFACE_CHANGED:
		ret = oppo_sla_iface_changed(nlh);
		break;

	case SLA_NOTIFY_PRIMARY_WIFI:
		ret = oppo_sla_set_primary_wifi(nlh);
		break;
	...
	case SLA_NOTIFY_WIFI_SCORE:
		ret = oppo_sla_set_wifi_score(nlh);
		break;
	...
	case SLA_NOTIFY_DUAL_STA_APP:
		oppo_sla_set_dual_wifi_app_uid(nlh);
		break;
	...
	case SLA_NOTIFY_VPN_CONNECTED:
		oppo_sla_set_vpn_state(nlh);
		break;

	case SLA_NOTIFY_DOWNLOAD_APP:
		oppo_sla_set_download_app_uid(nlh);
		break;

	case SLA_NOTIFY_VEDIO_APP:
		oppo_sla_set_vedio_app_uid(nlh);
		break;

	case SLA_LIMIT_SPEED_ENABLE:
		enable_oppo_limit_speed();
		break;

	case SLA_LIMIT_SPEED_DISABLE:
		disable_oppo_limit_speed();
		break;

	case SLA_LIMIT_SPEED_FRONT_UID:
		oppo_limit_uid_changed(nlh);
		break;
