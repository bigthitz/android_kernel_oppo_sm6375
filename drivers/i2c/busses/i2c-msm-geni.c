...
static int geni_i2c_probe(struct platform_device *pdev)
{
	...
	/* for levm skip auto suspend timer */
	if (!gi2c->is_le_vm) {
		dev_info(&pdev->dev, "enter i2c_audo_suspend_delay\n");
/* Function: Modify the I2C sleep time of Qualcomm CP communication,Solve the problem of intermittent
 * or slow charging caused by I2C sleep.
 * Parameters: Which I2C sleep time needs to be modified and oplus needs to be configured, i2c-auto-suspend-delay-ms in dtsi.
 * configuration:According to the platform configuration time, 3s is recommended.
 */
#ifdef OPLUS_FEATURE_CHG_BASIC
		ret = of_property_read_u32(pdev->dev.of_node, "oplus,i2c-auto-suspend-delay-ms", &i2c_audo_suspend_delay);
		if (ret == 0) {
			pm_runtime_set_autosuspend_delay(gi2c->dev, i2c_audo_suspend_delay);
			dev_info(&pdev->dev, "i2c_audo_suspend_delay %d ms\n", i2c_audo_suspend_delay);
		} else {
