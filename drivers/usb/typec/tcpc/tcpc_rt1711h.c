...
static struct rt1711_chip *g_chip = NULL;
...
static void pd_phy_set_continuous_time(struct rt1711_chip *chip)
{
	uint8_t data = 0x4E;
	rt1711_write_device(chip->client, 0xE7, 1, &data);
	data = 0x86;
	rt1711_write_device(chip->client, 0xE8, 1, &data);
	data = 0x3d;
	rt1711_write_device(chip->client, 0xE9, 1, &data);
	data = 0x3b;
	rt1711_write_device(chip->client, 0xEA, 1, &data);
	data = 0x10;
	rt1711_write_device(chip->client, 0xEB, 1, &data);
	pr_err("%s enter\n", __func__);
}

void ext_pd_phy_set_continuous_time(void)
{
	if (g_chip == NULL)
		return;
	pd_phy_set_continuous_time(g_chip);
}
EXPORT_SYMBOL(ext_pd_phy_set_continuous_time);
...
/********* workaround MO.230913213000256759: sc6607 workaround for pd abnormal start*********/
	...
	if (chip->chip_pid == SC6601_PID) {
		...
		if (data < 0)
			return data;
		if (en)
			data = data | TCPC_V10_REG_TCPC_CTRL_EN_WDT;
		else
			data = data & (~TCPC_V10_REG_TCPC_CTRL_EN_WDT);
		pr_info("%s %x = %x\n", __func__, TCPC_V10_REG_TCPC_CTRL, data);
		return rt1711_i2c_write8(tcpc, TCPC_V10_REG_TCPC_CTRL, data);
	} else {
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	rv = rt1711_get_chip_vid(tcpc, &chip_vid);
...
#endif
#ifdef OPLUS_FEATURE_CHG_BASIC
	chip->disable_irq_reset = of_property_read_bool(np, "rt,disable_irq_reset");
#endif
	return ret < 0 ? ret : 0;
}
