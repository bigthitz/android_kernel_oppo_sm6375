...
static int tcpci_alert_recv_msg(struct tcpc_device *tcpc)
{
	...
	rc = tcpci_get_chip_vid(tcpc, &chip_vid);
	if (!rc && chip_vid == SOUTHCHIP_PD_VID) {
		is_southchip_ic = true;
		rc = tcpci_get_chip_pid(tcpc, &chip_pid);
	}

	if (!rc && (SC2150A_PID == chip_pid) && (tcpc->pd_bist_mode == PD_BIST_MODE_DISABLE))
...
static int tcpci_alert_fault(struct tcpc_device *tcpc)
{
	...
	if (fault_status & VCONN_OCP_FAULT) {
