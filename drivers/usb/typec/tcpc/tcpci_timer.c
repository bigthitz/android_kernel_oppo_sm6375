...
DECL_TCPC_TIMEOUT_RANGE(PD_TIMER_SENDER_RESPONSE, 22, 28),
DECL_TCPC_TIMEOUT_RANGE(PD_TIMER_SENDER_RESPONSE_30, 25, 31),
...
DECL_TCPC_TIMEOUT_RANGE(PD_TIMER_CK_NOT_SUPPORTED, 40, 40),
...
DECL_TCPC_TIMEOUT(PD_TIMER_VSAFE0V_TOUT, 710),
...
DECL_TCPC_TIMEOUT_RANGE(TYPEC_TIMER_SRCDISCONNECT, 0, 0),
		...
		struct tcpc_device *tcpc, uint32_t timer_id)
{
	...
	if (timer_id == PD_TIMER_SENDER_RESPONSE_30)
		timer_id = PD_TIMER_SENDER_RESPONSE;
...
void tcpc_enable_timer(struct tcpc_device *tcpc, uint32_t timer_id)
{
	...
	if (pd_check_rev30(&tcpc->pd_port) && timer_id == PD_TIMER_SENDER_RESPONSE)
		timer_id = PD_TIMER_SENDER_RESPONSE_30;
...
int tcpci_timer_init(struct tcpc_device *tcpc)
{
	...
	for (i = 0; i < PD_TIMER_NR; i++) {
		...
		pr_info("name [%s] -> %d\n", tcpc_timer_desc[i].name, tcpc_timer_desc[i].tout);
	}
